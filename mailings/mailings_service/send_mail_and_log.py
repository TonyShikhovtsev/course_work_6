...
def send_mail_and_log(**kwargs):
    ...
    if new_mailing.status == 'enabled' and new_mailing.start_time <= current_time:
        try:
            ...
            status = 'Ошибка'
            error_message = f'Не удалось подключиться к SMTP-серверу: {e}'
        except smtplib.SMTPAuthenticationError as e:
            status = 'Ошибка'
            error_message = 'Ошибка аутентификации в почтовом сервисе'
        except smtplib.SMTPException as e:
            status = 'Ошибка'
            error_message = f'Ошибка при отправке письма: {e}'
        finally:
            print(f'Sending mail: Status={status}, Error={error_message}')
            Logs.objects.create(
                user=user,
        ...
        print('Mailing is not enabled or start time is in the future')
...
def cronjob_send_mail_and_log(**kwargs):
    ...
    try:
        ...
        print(f'Sending mail (cron): Status={status}, Error={error_message}')
        Logs.objects.create(
            user=user,
            last_attempt_time=current_time,
            status=status,
